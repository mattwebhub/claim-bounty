...
This package contains public status records, the cleared sealed comparator record, and a public-safe record of the unscored ClaimBounty exploratory attempt. It does not contain frozen case files, restricted manifest details, an answer key, the prior assessment bundle, customer material, screenshots, raw browser traces, authentication or session metadata, raw run identifiers, or grader-only material.
...
- `claimbounty-exploratory-attempt.json` records the single parent invocation, elapsed time, blocked execution, exact-input failure, and out-of-contract contamination without publishing the raw run identifier, private paths, case files, grader-only material, or superseded internal seal hashes.
