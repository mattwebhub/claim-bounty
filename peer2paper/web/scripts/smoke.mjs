...
try {
  ...
  await expectPage("/en", "Find the weakness");
  await expectPage("/en", "/peer2paper-fox-loupe.png");
  ...
  await expectImage("/icon.png");
  await expectImage("/peer2paper-fox-loupe.png");
...
async function expectImage(route) {
  const response = await fetch(`${origin}${route}`);
  if (!response.ok) throw new Error(`${route}: expected 2xx, got ${response.status}`);
  if (!response.headers.get("content-type")?.startsWith("image/")) {
    throw new Error(`${route}: expected an image content type, got ${response.headers.get("content-type")}`);
  }
  expectSecurityHeaders(route, response);
}
