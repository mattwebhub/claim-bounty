# ROUTINE: Run ClaimBounty Scientific Audit
...
| execution-policy                   | parameter | Caller     | -                                                   | dispatch | json      | Frozen execution rules for runtime, attempt-local working directories, disabled analysis networking, operational memory and storage defaults, a 600-second command timeout, source immutability, source access, and internal-only release. |
| canonical-schema-bundle            | resource  | Definition | project://claimbounty/config/canonical-schemas.json | dispatch | file      | Routine-owned versioned schemas for canonical findings, claims, actions, limitations, and frozen audit objects.                                                                                                                            |
| audit-template                     | resource  | Definition | project://claimbounty/config/audit-template.html    | dispatch | file      | Project-owned pinned template used for the researcher-facing HTML report.                                                                                                                                                                  |
| audit-schema                       | resource  | Definition | project://claimbounty/config/audit-schema.json      | dispatch | file      | Routine-owned pinned JSON Schema for the machine-readable audit.                                                                                                                                                                           |
| execution-runner                   | resource  | Definition | project://claimbounty/execution                     | dispatch | directory | Project-owned versioned scientific execution runner used by reproduction, robustness, and independent statistical verification. Missing runner files block execution.                                                                      |
...
1. **Build compact study case**: Validate the target, map only its document and data dependency closure, preserve exact source-backed transformations and document conflicts, and freeze one study case containing primary_target and analysis_contract.
   ...
   - sub-routine: claim-bounty-operations/build-and-freeze-study-case
...
2. **Preflight and reproduce primary result**: Preflight the real case bundle and shared execution capability, run the contract-defined model unchanged, test bounded working-directory or input-binding repairs when needed, and preserve complete scientific outputs in one reproduction package.
   ...
   - completion: The runner verifies the frozen execution-policy digest plus live network and host-file denial before analysis. Missing or failed isolation emits not_executable. Memory and storage remain operational defaults, and unavailable exact enforcement does not emit not_executable by itself.
   ...
   - sub-routine: claim-bounty-operations/reproduce-original-result
   - child-input: execution-policy | parent-input | execution-policy
   - child-input: scientific-policy | parent-input | scientific-policy
...
3. **Decide whether to continue after reproduction**: Review a partial reproduction or unresolved numerical mismatch and apply the caller's explicit authorization boundary before expensive downstream analysis.
   ...
   - agent: claim-bounty-operations/claimbounty-orchestrator
...
4. **Test bounded sensitivity candidates**: Run three result-blind design lanes, rank no more than five candidates, always materialize the conditional second-review artifact, and execute approved analyses through the ready reproduction profile.
   ...
   - sub-routine: claim-bounty-operations/stress-test-analysis
   - child-input: execution-policy | parent-input | execution-policy
   - child-input: scientific-policy | parent-input | scientific-policy
   - child-input: execution-runner | parent-input | execution-runner
   - child-input: frozen-study-case-resource | parent-artefact | frozen-study-case
...
5. **Answer targeted evidence questions**: Run beside sensitivity work after reproduction succeeds or is explicitly continued. Answer no more than three questions raised by the claim, reproduction, or supplied-document conflicts, with no more than three deep-source checkpoints.
   ...
   - sub-routine: claim-bounty-operations/research-methods-and-evidence
   - child-input: audit-request | parent-input | audit-request
   - child-input: execution-policy | parent-input | execution-policy
   - child-input: scientific-policy | parent-input | scientific-policy
   - child-input: frozen-study-case-resource | parent-artefact | frozen-study-case
...
6. **Verify decisive findings**: Wait for sensitivity and targeted research, then independently rerun the frozen analysis contract from the real case bundle in a clean workspace with equivalent isolation and resource controls, reopen verdict sources, and produce adjudicated findings.
   ...
   - sub-routine: claim-bounty-operations/verify-and-adjudicate-findings
   - child-input: audit-request | parent-input | audit-request
   - child-input: execution-policy | parent-input | execution-policy
   - child-input: scientific-policy | parent-input | scientific-policy
   - child-input: execution-runner | parent-input | execution-runner
...
7. **Publish JSON-first audit package**: Build one canonical JSON audit from the verified findings and complete manuscript recommendations, render concise HTML from that JSON, and package replay evidence and scoped release checks.
   ...
   - sub-routine: claim-bounty-operations/assemble-final-audit
   - child-input: audit-request | parent-input | audit-request
   - child-input: execution-policy | parent-input | execution-policy
...
| reproduction-inputs            | Frozen reproduction contract and inputs         | project://claimbounty/audits/{runId}/reproduction/reproduction-inputs.json             | json      |
| execution-readiness            | Execution readiness                             | project://claimbounty/audits/{runId}/reproduction/execution-readiness.json             | json      |
| execution-profile              | Execution profile attempt                       | project://claimbounty/audits/{runId}/reproduction/execution-profile.json               | json      |
| primary-run                    | Untouched primary run checkpoint                | project://claimbounty/audits/{runId}/reproduction/checkpoints/primary-run.json         | json      |
| primary-comparison             | Primary numerical comparison                    | project://claimbounty/audits/{runId}/reproduction/checkpoints/primary-comparison.json  | json      |
| repair-candidates              | Frozen repair candidates                        | project://claimbounty/audits/{runId}/reproduction/checkpoints/repair-candidates.json   | json      |
| repair-attempt-1               | Repair attempt 1 checkpoint                     | project://claimbounty/audits/{runId}/reproduction/checkpoints/repair-attempt-01.json   | json      |
| repair-attempt-2               | Repair attempt 2 checkpoint                     | project://claimbounty/audits/{runId}/reproduction/checkpoints/repair-attempt-02.json   | json      |
| repair-attempt-3               | Repair attempt 3 checkpoint                     | project://claimbounty/audits/{runId}/reproduction/checkpoints/repair-attempt-03.json   | json      |
| secondary-target-1             | Secondary target 1 checkpoint                   | project://claimbounty/audits/{runId}/reproduction/checkpoints/secondary-target-01.json | json      |
| secondary-target-2             | Secondary target 2 checkpoint                   | project://claimbounty/audits/{runId}/reproduction/checkpoints/secondary-target-02.json | json      |
| secondary-target-3             | Secondary target 3 checkpoint                   | project://claimbounty/audits/{runId}/reproduction/checkpoints/secondary-target-03.json | json      |
| reproduction-evidence          | Per-target reproduction evidence                | project://claimbounty/audits/{runId}/reproduction/evidence                             | directory |
| reproduction-package           | Reproduction package                            | project://claimbounty/audits/{runId}/reproduction/reproduction-package.json            | json      |
...
| research-brief                 | Targeted research questions                     | project://claimbounty/audits/{runId}/research/research-brief.json                      | json      |
| screened-source-registry       | Screened source shortlist and slot plan         | project://claimbounty/audits/{runId}/research/screened-sources.json                    | json      |
| full-text-source-1             | Source evidence checkpoint 1                    | project://claimbounty/audits/{runId}/research/source-checkpoints/source-01.json        | json      |
| full-text-source-2             | Source evidence checkpoint 2                    | project://claimbounty/audits/{runId}/research/source-checkpoints/source-02.json        | json      |
| full-text-source-3             | Source evidence checkpoint 3                    | project://claimbounty/audits/{runId}/research/source-checkpoints/source-03.json        | json      |
| full-text-map                  | Assembled targeted full-text evidence           | project://claimbounty/audits/{runId}/research/full-text-map.json                       | json      |
| literature-evidence-package    | Targeted literature evidence package            | project://claimbounty/audits/{runId}/research/literature-evidence-package.json         | json      |
| literature-version-history     | Targeted evidence version history               | project://claimbounty/audits/{runId}/research/versions                                 | directory |
| verification-inputs            | Frozen important verification targets           | project://claimbounty/audits/{runId}/verification/verification-inputs.json             | json      |
| statistical-verification       | Independent statistical rerun                   | project://claimbounty/audits/{runId}/verification/statistical-verification.json        | json      |
| source-verification            | Independent source and document verification    | project://claimbounty/audits/{runId}/verification/source-verification.json             | json      |
| adjudication-package           | Verified finding package                        | project://claimbounty/audits/{runId}/verification/adjudication-package.json            | json      |
| manuscript-recommendations     | Concrete manuscript recommendations             | project://claimbounty/audits/{runId}/verification/manuscript-recommendations.json      | json      |
| audit-json                     | Canonical audit JSON                            | project://claimbounty/audits/{runId}/delivery/audit.json                               | json      |
| replay-package                 | Replay package                                  | project://claimbounty/audits/{runId}/delivery/replay                                   | directory |
| audit-html                     | Researcher audit report HTML                    | project://claimbounty/audits/{runId}/delivery/audit.html                               | html      |
| release-gates                  | Scoped release checks                           | project://claimbounty/audits/{runId}/delivery/release-gates.json                       | json      |
| audit-package                  | Audit package manifest                          | project://claimbounty/audits/{runId}/delivery/audit-package.json                       | json      |
